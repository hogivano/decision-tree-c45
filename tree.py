...
def getRow(dataset, classifier, state):
    arr = []
    return 0

def positive(dataset, classifier, state):
    index = 0
    total_rows = 0
    plus = 0

    for i in dataset.rows:
        if (i[classifier] == state):
            print(i[classifier])

    print(dataset.attributes)
    # for i in range(len(dataset.attributes)):
    #     print("data set attribute : ",  dataset.attributes[i],  " ", classifier)
    #     if (dataset.attributes[i] == classifier):
    #         index = i
    #         print("index value in if : ", i)
    #         break
    #
    # for item in dataset.rows:
    #     print("item in loop : ", item[index])
    #     if (item[index] == state):
    #         total_rows = total_rows + 1
    #         if (dataset.rows[dataset.class_col_index] == 1):
    #             plus = plus + 1

    # class_decision = dataset.datacsv[dataset.attributes[dataset.class_col_index]]
    # col_classifier = dataset.datacsv[classifier]
    # col_class_decision = dataset.datacsv[class_decision]
    #
    # plus = 0
    # for i in range(len(col_classifier)):
    #     if (col_classifier[i] == state):
    #         if (col_class_decision[i] == 1):
    #             plus = plus + 1
    #
    # for x in df:
    #     if (x == state):
...
def entropy(plus, minus, dataset):
...
def gain(entropy, dataset):

    return 0

def partitionSplit(percentage, dataset):
    indexTrain = int(len(dataset.datacsv) * percentage / 100)
    indexTest = len(dataset.datacsv) - indexTrain
    print("length datacsv : ", len(dataset.datacsv))
    print(indexTest)

def confusionMatrix(dataset, correctly, incorrectly):
    confusionMatrix = [2][2]
...
def initialize():
    # rule attributes
    rule_attributes = [6.8, 0.26, 0.32, 5.2, 0.043, 34.0, 134.0, 0.9937, 3.18, 0.47, 10.4   ]
    dataset = csvdata("")
    ...
    print ("classifier is ",  dataset.class_col_index)
    print(classifier)

    preprocessing(dataset)
    # prepocessing data

    for index_item in range(len(dataset.rows)):
        for i in range(len(rule_attributes)):
            if (float(dataset.rows[index_item][i]) < rule_attributes[i]):
                dataset.rows[index_item][i] = 0
            else:
                dataset.rows[index_item][i] = 1

    arr = []
    for i in range(len(dataset.rows)):
        arr.append(dataset.rows[i][11])
    print(arr)
    # preprocessing the dataset
    # print("positive : ",  positive(dataset, "fixed acidity", 0))



    # # initial data set, train and test data
    # dataset = csvdata("")
    # training_set = csvdata("")
    # test_set = csvdata("")
    #
    ...
    # attribute.remove("quality")
    # attribute_types = ['true', 'true', 'true', 'true', 'true', 'true', 'true', 'true', 'true', 'true', 'true', 'false']
    #
    # #text case median and data
    #
    # # preprocessing data by rule
    # for x in range(len(attribute)):
    #     df.loc[df[attribute[x]] >= rule_attributes[x], attribute[x]] = 1
    #     df.loc[df[attribute[x]] < rule_attributes[x], attribute[x]] = 0
    #
    # quality = df['quality'].tolist()
    # fixed_acidity = df['fixed acidity'].tolist()
    #
    # # atrribute
    ...
    # print( "classifier is ", dataset.class_col_index)
    #
    # print("test case : ", df.loc[df["quality"] == 1])
    #
    # print(positive(dataset, "fixed acidity", 0))
    #
    # # plusS = positive(dataset, "quality")
    # # print("Plus data : ", plusS, " Minus data : ", len(df) - plusS)
    #
    # val = input("Input percentage split : ")
    # val = int(val)
    # while (val <= 0 or val > 90):
    #     print("Range persentage split : 10 - 90")
    #     val = input("Percentage splite : ")
    #
    # partitionSplit(val, dataset)
    #
    # # for attr in attribute:
    # #     for (item in df[attr]):
    # #         plus = positive(len(item))
    # #         minux = len(j) - plus
    # #     entropy(plus, minus)
    #
    # # print(df)
    # numerator = 0
    # fixed_acidity.sort()
    # for fixed in fixed_acidity:
    #      numerator+=fixed
    #
    # print("len fixed acidity", len(fixed_acidity))
    # print(fixed_acidity[(int)(len(fixed_acidity)/2)])
    #
    # # print(quality)
    #
    # plus = 0
...
def preprocessing(dataset):
    ...
    rule_attributes = [6.8, 0.26, 0.32, 5.2, 0.043, 34.0, 134.0, 0.9937, 3.18, 0.47, 10.4, 6.0]
...
def run_decision_tree():
    ...
    entropy_total = dataset.attributes[len(dataset.attributes) - 1]
    # print(entropy_total.value_counts().toList())
